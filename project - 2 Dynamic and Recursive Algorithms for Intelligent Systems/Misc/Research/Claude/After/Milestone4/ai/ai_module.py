...
class AIModule:
    ...
    def similarity_score(self, q1, q2):
        return SequenceMatcher(None, q1.lower(), q2.lower()).ratio()

    
    def progressive_fuzzy_match(self, query, knowledge_base, thresholds=[0.9, 0.8, 0.75, 0.7]):
        cleaned_query = self.clean_query(query)

        # Try exact match first (ignore punctuation)
        for qa in knowledge_base:
            if self.clean_query(qa['question']) == cleaned_query:
                return qa['question'], qa['answer'], 1.0

        # Progressive fuzzy match
        best_match = None
        best_score = 0

        for threshold in thresholds:
            for qa in knowledge_base:
                score = self.similarity_score(self.clean_query(qa['question']), cleaned_query)
                if score >= threshold and score > best_score:
                    best_match = (qa['question'], qa['answer'], score)
                    best_score = score
            if best_match:
                break  # Stop if a good enough match is found

        return best_match
    
    def fuzzy_match_threshold(self, query: str, qa_cache: List[dict], threshold: float) -> Optional[str]:
        """
        Try to find the closest matching question from the QA cache using fuzzy matching.
        Returns the answer if a match above the threshold is found; otherwise, returns None.
        """
        best_score = 0.0
        best_answer = None
