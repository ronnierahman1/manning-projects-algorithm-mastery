    ...
    **Singleton-like**: Manages shared state for analytics and caching
    ...
    def fuzzy_match(self, query: str, threshold: float = 0.6, max_results: int = 1) -> List[Tuple[Dict[str, Any], float]]:
        ...
        **Accuracy**: Provides similarity scores from 0.0 (no match) to 1.0 (exact match)
        ...
        - Wrap entire method in try-catch block
        
        Step 2: INPUT VALIDATION
        - Check if query is empty or None using early return pattern
        - If invalid, return empty list: []
        - Normalize the query using self.normalize_text(query)
        - If normalized query is empty, return empty list: []
        
        Step 3: INITIALIZE RESULTS COLLECTION
        - Create empty list called 'matches' to store results
        
        Step 4: GET ALL AVAILABLE QA PAIRS
        - Use getattr() to safely get qa_pairs with empty list default: getattr(self, 'qa_pairs', [])
        - Use getattr() to safely get custom_qa_pairs with empty list default: getattr(self, 'custom_qa_pairs', [])
        - Combine both lists using + operator into variable 'all_qa_pairs'
        
        Step 5: CORE MATCHING LOOP
        - Loop through each 'qa' in 'all_qa_pairs'
        - For each QA pair:
          a) Validate QA structure: if not qa or 'question' not in qa: continue
          b) Normalize the stored question: question_normalized = self.normalize_text(qa['question'])
          c) Calculate similarity using difflib:
             - Import difflib
             - Create SequenceMatcher: difflib.SequenceMatcher(None, query_normalized, question_normalized)
             - Get similarity score: .ratio()
          d) Apply threshold filter: if similarity >= threshold:
             - Append tuple (qa, similarity) to matches list
        
        Step 6: SORT AND LIMIT RESULTS
        - Sort matches by similarity score (descending): matches.sort(key=lambda x: x[1], reverse=True)
        - Return limited results: matches[:max_results]
        
        Step 7: ERROR HANDLING
        - In except block:
          * Print error message: f"Error in fuzzy_match: {e}"
          * Return empty list: []
        
        IMPORTANT NOTES:
        - difflib.SequenceMatcher(None, a, b).ratio() returns float between 0.0 and 1.0
        - lambda x: x[1] extracts the similarity score (second element) from each tuple
        - reverse=True sorts from highest to lowest similarity
        - Always return empty list on error to maintain consistent interface
        """
    ...
    def fuzzy_match_with_thresholds(self, query: str, thresholds: List[float] = None) -> Optional[Tuple[str, float]]:
        ...
        **Low Thresholds (0.5-0.6)**: High recall, lower precision
        ...
        Step 1: DEFAULT THRESHOLDS SETUP
        - Check if thresholds parameter is None
        - If None, set thresholds = self.DEFAULT_FUZZY_THRESHOLDS
        
        Step 2: ERROR HANDLING SETUP
        - Wrap main logic in try-catch block
        
        Step 3: CASCADING THRESHOLD LOOP
        - Loop through each 'threshold' in 'thresholds' list
        - For each threshold:
          a) Call self.fuzzy_match(query, threshold, max_results=1) and store result in 'matches'
          b) Check if matches exist and has content: if matches and len(matches) > 0:
             - Extract first match: qa_dict, similarity = matches[0]
             - Update usage statistics: self._update_usage_stats(qa_dict)
             - Extract answer: answer = qa_dict.get('answer', '')
             - Return tuple: (answer, similarity)
        
        Step 4: NO MATCHES FOUND
        - If loop completes without finding matches, explicitly return None
        
        Step 5: ERROR HANDLING
        - In except block:
          * Print error message: f"Error in fuzzy_match_with_thresholds: {e}"
          * Return None (critical: never return empty tuple or other type)
        
        IMPORTANT NOTES:
        - The method tries thresholds from highest to lowest (most strict to most permissive)
        - matches[0] gets the best match because fuzzy_match sorts by similarity
        - Always return None on error to maintain Optional[Tuple[str, float]] type contract
        - self._update_usage_stats() tracks which QA pairs are being used for analytics
        - The explicit tuple (answer, similarity) ensures consistent return format
        """
        # TODO: Implement the method following the steps above
        pass    
