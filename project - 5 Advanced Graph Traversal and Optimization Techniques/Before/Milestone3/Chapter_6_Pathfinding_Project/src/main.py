...
from algorithms.dijkstra import dijkstra  # You will implement this!
...
def main():
    ...
    def generate_cost_map(grid):
        """
        Generates a simple cost map with arbitrary variation.
        Learners can experiment with modifying the cost logic.
        """
        cost_map = {}
        for r in range(len(grid)):
            for c in range(len(grid[0])):
                cost_map[(r, c)] = 1 + (r + c) % 3
        return cost_map
    ...
    for level, grid in terrain.items():
        ...
        # Run BFS (already implemented)
        bfs_path = bfs(grid, start, goal)

        # Run DFS (already implemented in previous milestone)
        dfs_path = dfs(grid, start, goal)

        # Run Dijkstra’s Algorithm (you will implement this!)
        # Write your code here: Add Dijkstra's path
        

        # Store results for rendering
        grids_info.append((level, "BFS", grid, bfs_path))
        grids_info.append((level, "DFS", grid, dfs_path))
        # Write your code here: Add Dijkstra's path to grids_info

    cell_size = 40
    padding = 40
    screen_width = (cell_size * 5 + padding) * 3 + padding
    screen_height = (cell_size * 5 + padding * 3) * 3
